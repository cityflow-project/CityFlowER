...
namespace CityFlow {
    ...
    double Vehicle::getCarFollowSpeed(double interval) {
        ...
        auto tensor = torch::tensor ({leader->getSpeed(), leader->getMaxNegAcc(),
                                                ...
                                                vehicleInfo.headwayTime, hasSet, customSpeed});
        tensor = tensor.view({1, -1});

        input.push_back(tensor);
        static torch::jit::script::Module module = torch::jit::load("./Imitate_cityflow/trained_rule.pt");
